...
parser.add_argument("--model", type=str, default="mixtral", help='choose model from llama2, mistral, mixtral.')
...
huggingface_hub.login('XXXXX')  # Your own HuggingFace Hub token
...
def validation(name, test_dataloader):
    ...
    for i, batch in bar:
        with torch.no_grad():
            ...
            print('sample ' + str(i) + ': ',
                  batch['text'][0][len(instructions_['instruction']):-len(instructions_['end'])])
